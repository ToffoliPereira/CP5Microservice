...
# Serviço de CEP com Via CEP

### 1. **Entidade CEP (Domain)**
Classe `Cep` com as seguintes propriedades:
- `Id` (int)
...
Interface `ICepRepository` e implementação `CepRepository` com métodos:
...
Interface `ICepService` e implementação `CepService` com métodos:
- `ConsultarCepAsync(string cep)` - Consultar Via CEP e salvar no banco
- `GetAllCepsAsync()` - Buscar todos os CEPs salvos

### 4. **Controller CEP (API)**
`CepController` com endpoints:
