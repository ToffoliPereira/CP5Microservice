...
namespace performance_cache.Controllers
{
    ...
    public class EstoqueController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                ...
                if (estoqueList == null || !estoqueList.Any())
                {
                    ...
                    return Ok(new List<MovimentacaoEstoque>());
                }
        ...
        // POST: api/vehicle
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] MovimentacaoEstoque estoque)
        {
            try
            {
                if (estoque == null)
                ...
                if (string.IsNullOrWhiteSpace(estoque.Tipo) ||
                    string.IsNullOrWhiteSpace(estoque.Qtd.ToString()) ||
                    string.IsNullOrWhiteSpace(estoque.Lote))
                {
                ...
                logger.LogInformation("Criando novo estoque: {Tipo} {Qtd} - {Lote}", estoque.Tipo, estoque.Qtd, estoque.Lote);
        ...
        // PUT: api/vehicle/{id}
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] MovimentacaoEstoque estoque)
        {
            try
            {
                if (id <= 0)
                ...
                estoque.Id = id;
                logger.LogInformation("Atualizando estoque ID: {Id} - {Tipo} {Qtd} - {Lote}", id, estoque.Tipo, estoque.Qtd, estoque.Lote);
